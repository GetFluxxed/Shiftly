...
export function TodayScreen() {
  ...
  return <Screen title={`${greeting},\n${firstName}.`} eyebrow={new Date().toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}
    ...
    <Card style={styles.storeCard}>
      <View style={layout.row}><Ionicons name="storefront-outline" size={25} color={colors.white} />
      ...
      {stores.length > 1 ? <Pressable accessibilityRole="button" accessibilityLabel="Change your store" onPress={() => router.push('/accounts')}
        ...
        <Text style={styles.storeSwitchText}>Change store</Text><Ionicons name="swap-horizontal" size={20} color={colors.white} />
      </Pressable> : null}
    </Card>
    <Columns><Column>
      <Card>
      ...
      {reports ? <Card>
        <View style={layout.row}><Ionicons name="megaphone-outline" color={colors.coral} size={24} /><Heading>Heads up</Heading></View>
      ...
      </Card> : <Card style={{ backgroundColor: colors.sage }}><Heading>One team. Your own account.</Heading>
...
const styles = StyleSheet.create({
  storeCard: { backgroundColor: colors.forest, borderColor: colors.forest, gap: 16 },
  storeLabel: { color: '#C3D4BE', fontSize: 11, letterSpacing: 1.5, fontWeight: '700', marginBottom: 7 },
  storeName: { color: colors.white, fontSize: 25, fontWeight: '600', lineHeight: 32 },
  storeDescription: { color: '#E4EBDF', fontSize: 15, lineHeight: 23 },
  storeSwitch: { minHeight: 48, alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, paddingRight: 12 },
  storeSwitchText: { color: colors.white, fontSize: 15, fontWeight: '700' },
});
